...
{
public:
    std::vector<PredictResult> predict_results; // AI推理结果
    ...
    Detection(const std::string pathModel)
    {
        ...
        if (!ifs.is_open()) {
            std::cerr << "Failed to open file: " << io_paddle << std::endl;
            return; // 或适当的错误处理
        }
    ...
    void render()
    {
        ...
        for (int i = 0; i < prod; i += 6)
        {
            ...
            predict_results.push_back(result);
        }
    }
    ...
    void drawBox(Mat &img)
    {
        ...
        {
            PredictResult result = predict_results[i];
